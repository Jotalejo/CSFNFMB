...
class Cita(Base):
    ...
    id = Column("cod_cita", Integer, primary_key=True,
                autoincrement=True, index=True)

    # FK opcionales (deja los ForeignKey si ya tienes esas tablas; si no, puedes quitarlos)
    clipot = Column("codclipot_cita", Integer, ForeignKey(
        "clientespot.cod_clipot"), nullable=True, index=True)
    fechacita = Column("fec_cita", Date, nullable=True)
    horacita = Column("hora_cita", Time, nullable=True)
    cliente = Column("codcli_cita", Integer, ForeignKey(
        "clientes.cod_cli"), nullable=True, index=True)

    asuntocita = Column("asunto_cita", String(200), nullable=True)
    tipodevisi = Column("codtipovis_cita", Integer, ForeignKey(
        "tiposvisita.cod_tipovis"), nullable=True, index=True)
    ...
    tipogen = Column("codtipogen_cita", Integer, ForeignKey(
        "tipogenerador.cod_tipogen"), nullable=True, index=True)
